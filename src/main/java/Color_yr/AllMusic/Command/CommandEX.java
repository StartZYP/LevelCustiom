...
public class CommandEX {
    ...
    private static void AddMusic(Object sender, String Name, String[] args) {
        ...
        if (args[0].contains("id=") && !args[0].contains("/?userid")) {
            if (args[0].contains("&user")) {
                ...
                MusicID = Function.getString(args[0], "id=", (String)null);
            }
        } else if (args[0].contains("song/")) {
            if (args[0].contains("/?userid")) {
                MusicID = Function.getString(args[0], "song/", "/?userid");
            } else {
                MusicID = Function.getString(args[0], "song/", (String)null);
            }
        } else {
            MusicID = args[0];
        }
            ...
            if (PlayMusic.getSize() >= AllMusic.getConfig().getMaxList()) {
                ...
                if (AllMusic.Side.NeedPlay()) {
                    AllMusic.Side.RunTask(() -> {
                        PlayMusic.addMusic(MusicID, Name, false);
                    });
    ...
    private static void ShowSearch(Object sender, SearchPage search) {
        ...
        if (search.haveLastPage()) {
            AllMusic.Side.SendMessage(sender, AllMusic.getMessage().getPage().getLast(), Action.RUN_COMMAND, "/music lastpage");
        }

        for(int a = 0; a < index; ++a) {
            SearchOBJ item = search.getRes(a + search.getPage() * 10);
            String info = AllMusic.getMessage().getPage().getChoice();
            info = info.replace("%index%", "" + (a + 1)).replace("%MusicName%", item.getName()).replace("%MusicAuthor%", item.getAuthor()).replace("%MusicAl%", item.getAl());
            AllMusic.Side.SendMessage(sender, info, Action.RUN_COMMAND, "/music select " + (a + 1));
        }

        if (search.haveNextPage()) {
            AllMusic.Side.SendMessage(sender, AllMusic.getMessage().getPage().getNext(), Action.RUN_COMMAND, "/music nextpage");
        }
    ...
    public static void EX(Object sender, String Name, String[] args) {
        if (args.length == 0) {
            ...
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music [音乐ID] 来点歌§e[§n点我§r§e]", Action.SUGGEST_COMMAND, "/music ");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music stop 停止播放歌曲§e[§n点我§r§e]", Action.RUN_COMMAND, "/music stop");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music list 查看歌曲队列§e[§n点我§r§e]", Action.RUN_COMMAND, "/music list");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music vote 投票切歌§e[§n点我§r§e]", Action.RUN_COMMAND, "/music vote");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music nomusic 不再参与点歌§e[§n点我§r§e]", Action.RUN_COMMAND, "/music nomusic");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music search [歌名] 搜索歌曲§e[§n点我§r§e]", Action.SUGGEST_COMMAND, "/music search ");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music select [序列] 选择歌曲§e[§n点我§r§e]", Action.SUGGEST_COMMAND, "/music select ");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music hud enable [位置] 启用关闭Hud§e[§n点我§r§e]", Action.RUN_COMMAND, "/music vv enable");
            AllMusic.Side.SendMessage(sender, "§d[AllMusic]§2使用/music hud [位置] [x] [y] 设置某个Hud的位置", Action.SUGGEST_COMMAND, "/music vv ");
            ...
            if (args[0].equalsIgnoreCase("list")) {
                if (PlayMusic.NowPlayMusic != null && !PlayMusic.NowPlayMusic.isNull()) {
                    data = AllMusic.getMessage().getMusicPlay().getPlay();
                    data = data.replace("%MusicName%", PlayMusic.NowPlayMusic.getName()).replace("%MusicAuthor%", PlayMusic.NowPlayMusic.getAuthor()).replace("%MusicAl%", PlayMusic.NowPlayMusic.getAl()).replace("%MusicAlia%", PlayMusic.NowPlayMusic.getAlia()).replace("%PlayerName%", PlayMusic.NowPlayMusic.getCall());
                    AllMusic.Side.SendMessage(sender, data);
                } else {
                ...
                if (PlayMusic.getSize() == 0) {
                    ...
                    AllMusic.Side.SendMessage(sender, AllMusic.getMessage().getMusicPlay().getListMusic().getHead().replace("&Count&", "" + PlayMusic.getSize()));
                ...
                if (PlayMusic.getSize() == 0 && AllMusic.getConfig().getPlayList().size() == 0) {
                    ...
                    data = AllMusic.getMessage().getVote().getBQ();
                    ...
                    if (!AllMusic.containVote(Name)) {
                        ...
                        data = AllMusic.getMessage().getVote().getBQAgree();
                        data = data.replace("%PlayerName%", Name).replace("%Count%", "" + AllMusic.getVoteCount());
            ...
            } else if (args[0].equalsIgnoreCase("ban") && args.length == 2 && AllMusic.getConfig().getAdmin().contains(Name)) {
            ...
            } else if (args[0].equalsIgnoreCase("delete") && args.length == 2 && AllMusic.getConfig().getAdmin().contains(Name)) {
            ...
            } else if (args[0].equalsIgnoreCase("addlist") && args.length == 2 && AllMusic.getConfig().getAdmin().contains(Name)) {
                if (Function.isInteger(args[1])) {
            ...
            } else if (args[0].equalsIgnoreCase("clearlist") && AllMusic.getConfig().getAdmin().contains(Name)) {
                ...
                if (args[0].equalsIgnoreCase("select") && args.length == 2) {
                    ...
                    if (obj == null) {
                        ...
                        String[] ID = new String[]{obj.GetSong(obj.getPage() * 10 + (a - 1))};
                    ...
                    if (args.length == 1) {
                        ...
                        if (args.length == 3) {
                            try {
                                temp = Hud.SetHudEnable(Name, args[2]);
                                AllMusic.Side.SendMessage(sender, AllMusic.getMessage().getHud().getState().replace("%State%", temp ? "启用" : "关闭").replace("%Hud%", args[2]));
                            } catch (Exception var7) {
                                AllMusic.Side.SendMessage(sender, AllMusic.getMessage().getCommand().getError());
                            }
                        } else {
                            temp = Hud.SetHudEnable(Name, (String)null);
                            AllMusic.Side.SendMessage(sender, AllMusic.getMessage().getHud().getState().replace("%State%", temp ? "启用" : "关闭").replace("%Hud%", "所有"));
                        }
                        ...
                        try {
                            ...
                            if (obj == null) {
                                ...
                                String temp = AllMusic.getMessage().getHud().getSet().replace("%Hud%", args[1]).replace("%x%", args[2]).replace("%y%", args[3]);
